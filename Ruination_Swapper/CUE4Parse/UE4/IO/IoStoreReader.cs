...
namespace CUE4Parse.UE4.IO
{
    public class IoStoreReader : AbstractAesVfsReader
    {
        ...
        public List<string> ContainerStreamNames = new();
        ...
        public override byte[] Extract(VfsEntry entry)
        {
            ...
            return Read(ioEntry.Offset, ioEntry.Size, (int)ioEntry.TocEntryIndex);
        }
        ...
        public byte[] Read(FIoChunkId chunkId)
        {
            ...
            {
                return Read((long) offsetLength.Offset, (long) offsetLength.Length, (int) chunkId.ChunkId);
            }

            throw new KeyNotFoundException($"Couldn't find chunk {chunkId} in IoStore {Name}");
        }

        private byte[] Read(long offset, long length, int OffsetLengthIndex)
        {
          // if (CUtils.isExport) WebviewAppShared.Utils.Utils.MessageBox("Length: " + length); //2089
            ...
            for (int blockIndex = firstBlockIndex; blockIndex <= lastBlockIndex; blockIndex++)
            {
                ...
                string readerName = ContainerStreamNames[partitionIndex];

                using(FileStream fs = new FileStream(ContainerStreamNames[partitionIndex], FileMode.Open, FileAccess.Read, FileShare.Read))
                {
                    fs.Seek(partitionOffset, SeekOrigin.Begin);
                    fs.Read(compressedBuffer, 0, (int)rawSize);
                }
                ...
                else
                {
                    ...
                    Compression.Compression.Decompress(compressedBuffer, 0, (int) rawSize, uncompressedBuffer, 0,
                        (int) uncompressedSize, compressionMethod);
                    src = uncompressedBuffer;
                }

                var sizeInBlock = (int) Math.Min(compressionBlockSize - offsetInBlock, remainingSize);
                //65536 0 2089 2089
               // if (CUtils.isExport) WebviewAppShared.Utils.Utils.MessageBox(((int)compressionBlockSize) + " | " + dstOffset + " | " + offsetInBlock + " | " + remainingSize + " | " + sizeInBlock);
                ...
                if (CUtils.isExport)
                {
                    ...
                    if (TocResource.Header.PartitionCount is 0 or 1)
                    {
                        UcasFile = readerName;
                    }
                    else
                    {
                        string filePath = readerName;
                        string fileName = System.IO.Path.GetFileNameWithoutExtension(filePath);
                        filePath = filePath.SubstringBeforeLast(fileName);

                        string pakchunkName = fileName.Split("_s")[0];

                        pakchunkName += "_s" + (TocResource.Header.PartitionCount - 1) + ".ucas";

                        UcasFile = filePath + pakchunkName;
                    }

                    CUtils.Export = new CUtils.IoPackageExport()
                    {
                        Offset = partitionOffset,
                        Compressed = compressionBlock.CompressionMethodIndex != 0,
                        UtocFile = TocResource.Filename,
                        UcasFile = UcasFile,
                        UtocLOOffset = TocResource.ChunkOffsetLengthsPositions[OffsetLengthIndex],
                        UtocLOSize = length,
                        UtocCBlockOffset = TocResource.CompressionBlockPositions[blockIndex],
                        OffsetAndLengthEntry = TocResource.ChunkOffsetLengths[OffsetLengthIndex],
                        PartitionCount = (int)TocResource.Header.PartitionCount,
