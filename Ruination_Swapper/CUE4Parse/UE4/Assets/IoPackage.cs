...
namespace CUE4Parse.UE4.Assets
{
    ...
    public sealed class IoPackage : AbstractUePackage
    {
        ...
        public FZenPackageSummary zenSummary;
        public int PropertiesExportMapOffset;
        public byte[] PropertiesWithExportMap;
        public CUtils.IoPackageExport ExportData;
        public string[] NameMapAsStrings;
        public byte[] BulkDataArray;
        public ulong HashVersion;

        private byte[] OriginalCB;
        private byte[] OriginalLO;
            ...
            IFileProvider? provider = null, TypeMappings? mappings = null) : base(uasset.Name.SubstringBeforeLast('.'), provider, mappings)
        {
            ...
            if (uassetAr.Game >= EGame.GAME_UE5_0)
            {
                ...
                // Name map
                NameMap = FNameEntrySerialized.LoadNameBatch(uassetAr, ref this.HashVersion);
                ...
                BulkDataArray = Array.Empty<byte>();

                if (uassetAr.Ver >= EUnrealEngineObjectUE5Version.DATA_RESOURCES)
                {
                    int bulkStart = (int)uassetAr.Position;
                    ...
                    {
                        int bulkSize = bulkEnd - bulkStart;
                        uassetAr.Position = bulkStart;
                        BulkDataArray = uassetAr.ReadBytes(bulkSize);
                ...
                PropertiesExportMapOffset = (int)uassetAr.Position;
                PropertiesWithExportMap = uassetAr.ReadBytes(remainingFileLength);
            ...
            this.NameMapAsStrings = Array.Empty<string>();
            ExportData = CUtils.Export;
            ...
            if (this.NameMap.Length > 0)
            {
                ...
                for (int i = 0; i < this.NameMap.Length; i++)
                {
                    var nameMapEntry = this.NameMap[i];
                    this.NameMapAsStrings[i] = nameMapEntry.Name;
                }
            }
        ...
        public bool RemoveProtectedStrings()
        {
            string pathToRemove = this.Name;
            string fileNameToRemove = Path.GetFileNameWithoutExtension(this.Name);

            var nameMapAsList = this.NameMapAsStrings.ToList();

            if (!nameMapAsList.Contains(pathToRemove) || !nameMapAsList.Contains(fileNameToRemove))
            {
                return false;
            }

            nameMapAsList.Remove(pathToRemove);
            nameMapAsList.Remove(fileNameToRemove);

            this.NameMapAsStrings = nameMapAsList.ToArray();
        ...
        public bool ChangeProtectedStrings(List<string> NewProtectedStrings)
        {
            ...
            var currentProtectedStrings = this.GetProtectedStrings();

            if (currentProtectedStrings.Count != NewProtectedStrings.Count)
            {
                return false;
            }
            ...
            for (int i = 0; i < currentProtectedStrings.Count; i++)
            {
                string currentString = currentProtectedStrings[i];
                int indexInCurrent = nameMapAsList.IndexOf(currentString);
                nameMapAsList[indexInCurrent] = NewProtectedStrings[i];
            }
        ...
        private string revertableDetector = "RUINATIONSWAPPERREVERTABLE";

        public bool ChangePublicExportHash(IoPackage oldPackage)
        {
            this.ExportMap[this.ExportMap.Length - 1].PublicExportHash = oldPackage.ExportMap.Last().PublicExportHash;
            return true;
        }

        public bool LoadAlreadySwapped()
        {
            var ucasFileLength = new FileInfo(this.ExportData.UcasFile).Length;
            var ucasOffset = this.ExportData.Offset;
            var assetSize = AssetSize;

            if ((ucasOffset + assetSize) > ucasFileLength)
            {
                return false;
            }

            var revertableDetectorLength = Encoding.UTF8.GetBytes(revertableDetector).Length;

            revertableDetectorLength += 12; //CompressionBlock
            revertableDetectorLength += 10; //LengthOffset

            if((ucasOffset + assetSize + revertableDetectorLength) > ucasFileLength)
            {
                return false;
            }

            var assetWithPotentialDetector = new byte[assetSize + revertableDetectorLength];

            using (FileStream fs = new FileStream(this.ExportData.UcasFile, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                fs.Seek(ucasOffset, SeekOrigin.Begin);
                fs.Read(assetWithPotentialDetector, 0, (int)assetWithPotentialDetector.Length);
            }

            var potentialDetectorPart = assetWithPotentialDetector.Skip((int)assetSize - 1).ToArray();

            if(!Encoding.UTF8.GetString(potentialDetectorPart).StartsWith(revertableDetector))
            {
                return false;
            }

            var offsetBytes = potentialDetectorPart.Skip(revertableDetector.Length + 1).ToArray();

            var byteArchive = new FByteArchive("", offsetBytes);

            var originalcb = byteArchive.ReadBytes(12);
            var originallo = byteArchive.ReadBytes(10);

            this.OriginalCB = originalcb;
            this.OriginalLO = originallo;

            return true;
        }

        public byte[] GetOriginalCompressionBlock() => OriginalCB;

        public byte[] GetOriginalLengthOffset() => OriginalLO;
