    ...
    public sealed class IoPackage : AbstractUePackage
    {
            ...
            IFileProvider? provider = null, TypeMappings? mappings = null) : base(uasset.Name.SubstringBeforeLast('.'), provider, mappings)
        {
            ...
            if (uassetAr.Game >= EGame.GAME_UE5_0)
            {
                ...
                // Name map
                NameMap = FNameEntrySerialized.LoadNameBatch(uassetAr, ref HashVersion);
        ...
        public bool ChangeLastPublicExportHash(IoPackage oldPackage)
        {
            ulong newexporthash = oldPackage.ExportMap.Last().PublicExportHash;

            int exportMapOffset = this.zenSummary.ExportMapOffset + (72 * (this.ExportMap.Count()));
            int publicExportOffset = exportMapOffset - 16;

            byte[] exporthashBytes = BitConverter.GetBytes(newexporthash);

            for(int i = 0; i < exporthashBytes.Length; i++)
            {
                this.PropertiesWithExportMap[publicExportOffset + i - PropertiesExportMapOffset] = exporthashBytes[i];
            }
